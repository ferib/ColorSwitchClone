...
public class ParkourGenerator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        int CicleCount = (int)((MainCamera.transform.position.y - 0.0f ) / CirclesPerMeter);
        Debug.Log("Cicles needed: " + ((MainCamera.transform.position.y - 0.0f) / CirclesPerMeter));

        if(CicleCount > PrefabCircles.Count)
        {
            //Add circle
            GameObject pCircle = Instantiate(PrefabCircle) as GameObject;
            pCircle.transform.position = new Vector3(MainCamera.transform.position.x, MainCamera.transform.position.y + 5.0f, MainCamera.transform.position.z);

            //random scale
            float scaleFactor = Random.Range(100, 150) / 100.0f;
            pCircle.transform.localScale = new Vector3(scaleFactor, scaleFactor, 1.0f);
